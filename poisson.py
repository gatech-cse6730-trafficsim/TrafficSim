...
class trafficflow():
    def __init__(self, num_intersection=2, num_time=3, traffic_flow=[200,300,200], \
                 side_road = True, bidir = True, rand = 'exp'):
    ...
    def generate_arrive_time(self, param, rand = 'exp', time  = 0):
        ...
        if rand == 'exp':
            ...
            arrive_time = np.sort(np.random.uniform(0, 3600, param))
        else: # uniform time distribution
            arrive_time = np.linspace(1, 3600, param)
    ...
    def main(self):
        for i in range(self.time):
            ...
            if self.side_road:
                for i in range(self.intersections):
                    self.gen_traffic_flow_leftside(inter_idx = i, param = self.traffic_flow[i]*0.5, t= i)
                    self.gen_traffic_flow_rightside(inter_idx = i, param = self.traffic_flow[i]*0.5, t= i)
